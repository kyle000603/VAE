...
def prepare_data():
    train_data = datasets.MNIST(root='./', train=True, download=True, transform=transforms.Compose([transforms.Resize(64),
                                                                                                    transforms.ToTensor()]))
    test_data = datasets.MNIST(root='./', train=False, download=True, transform=transforms.Compose([transforms.Resize(64),
                                                                                                    transforms.ToTensor()]))
    
    train_loader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True, num_workers=12, prefetch_factor=32)
...
class UnFlatten(nn.Module):
    def forward(self, input:torch.Tensor, size=1024):
        return input.reshape(input.shape[0], size, 1, 1)

class VariationalAutoEncoder(nn.Module):
    def __init__(self, batch_size, hidden_size=8, hidden_dim=1024, input_size=784, VQ=False) -> None:
        ...
        self.VQ = VQ
        self.resize = transforms.Resize(24)
